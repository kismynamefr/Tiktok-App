...
const Nav = () => {
  ...
  const { usersHasSignin, serialId } = useContext(AuthContext);

  const handleOpenModal = () => {
    setOpenModal(true);
  };

  const fetchTippyUser = () => {
    return (
      <>
        <TopDmIcon>
          <a href="">
        ...
        <Tippy
          interactive
          render={(attrs) => (
            <UserPRFContainer tabIndex="-1" {...attrs}>
              <ProfilePopup>
                <ProfileInfo>
                  <StyledUserLink>
                    <svg
        ...
        >
          <ProfileContainer avatar={userProfile && userProfile.photoURL} />
          {/* Avatar Users */}
        </Tippy>
      </>
    );
  ...
  const fetchProfileUser = useCallback(async () => {
    if (usersHasSignin && serialId) {
      await axios({
        method: "get",
        url: `http://localhost:5506/users/serialid/${serialId}`,
      }).then((res) => {
...
const ProfileContainer = styled.div`
  ...
  cursor: pointer;
`;
const UserPRFContainer = styled.div`
  position: absolute;
  z-index: 1;
...
`;
const SignOutContainer = styled.li`
  padding: 10px 16px;
